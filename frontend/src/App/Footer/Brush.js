...
export default class Brush extends PureComponent {
  ...
  componentDidMount = () => {
    ...
    let xAxis = d3.axisBottom(xTotalScale).tickFormat(function(d){ return d.x;});
...
let updateCurrentlySelectedData = (range) => {
  dataStore.currentlySelectedData = rawData.slice(xTotalScale.invert(range[0]),
...
function brushed() {
  ...
  */
  ...
  udpateCurrentlySelectedRange(d3.event.selection);
